...
const CallbackJobsModal = ({ isOpen, onClose, data, tradeGroup }) => {
    if (!data) return null;
    ...
    return (
        <Modal
            ...
            onClose={onClose}
            noBackdrop
            title={`Callback Jobs — ${tradeGroup}`}
        >
            <div className="flex flex-col gap-4 p-2">
                ...
                <div className="grid grid-cols-1 gap-3">
                    <div className="p-3 rounded-xl bg-red-50 border border-red-200 text-center">
                        <div className="text-[10px] font-bold text-red-600 uppercase tracking-wider flex items-center justify-center gap-1">
                            <RotateCcw className="w-3 h-3" /> Total Callbacks
                        </div>
                        ...
                        <tbody>
                            {(data.callback_jobs || []).map((job, i) => (
                                <tr key={i} className="border-b border-black/5 transition-colors hover:bg-muted/20">
                                    ...
                                    <td className="p-3 font-bold text-foreground">{job.job_number}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                {(data.callback_jobs || []).length === 0 && (
                    <div className="text-center py-8 text-muted-foreground">
                        <RotateCcw className="w-10 h-10 mx-auto mb-2 opacity-30" />
                        <p className="font-medium">No callback jobs found.</p>
                    </div>
                )}
            </div>
        </Modal>
...
export default CallbackJobsModal;

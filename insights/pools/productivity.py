# insights/pools/productivity.py
...
def _impact_text(kpi: str) -> str:
    return {
        ...
        "Late to Site %": "Late arrivals can hurt customer experience and increase complaints.",
        "Absence %": "Higher absence reduces capacity and makes delivery harder.",
    }.get(kpi, "Productivity performance is below target and may reduce output and revenue.")
...
def _action_text(kpi: str) -> Dict[str, str]:
    return {
        ...
        "Late to Site %": {"level": "MEDIUM", "text": "Reduce lateness: improve scheduling buffers, routing, and early customer updates."},
        "Absence %": {"level": "MEDIUM", "text": "Reduce absence impact: early intervention, wellbeing support, and better cover planning."},
...
def _kpi_month_scores(month_payloads: List[Dict[str, Any]]) -> Dict[str, List[Optional[float]]]:
    out: Dict[str, List[Optional[float]]] = {k: [] for k in PRODUCTIVITY_KPIS}
    for payload in (month_payloads or []):
        scores = (payload or {}).get("kpi_scores") or {}
        for kpi in PRODUCTIVITY_KPIS:
...
def build_productivity_quarterly_insights(
    *,
    ...
    improved = sorted([k for k in PRODUCTIVITY_KPIS if _is_number(changes.get(k)) and changes[k] > 0])
    worsened = sorted([k for k in PRODUCTIVITY_KPIS if _is_number(changes.get(k)) and changes[k] < 0])

    qres = quarter_result or {}
    q_pool = _safe_float(((qres.get("category_scores") or {}).get("Productivity")))
    ...
    if consistently_low:
        summary = (
            f"Over {window_label}, productivity was held back by a few areas that were below target in at least 2 of the 3 months: "
            f"{', '.join(consistently_low)}."
        )
    else:
        summary = f"Over {window_label}, productivity did not show any repeated weak areas across the 3 months."

    key_findings: List[str] = []
    key_findings.append(f"Months analysed: {window_label}.")
    key_findings.append(f"Productivity score for the 3-month period: {_fmt(q_pool,1)} (overall score: {_fmt(q_overall,1)}).")
    if bonus_sentence:
    ...
    if consistently_low:
        ...
        key_findings.append("No productivity KPI was repeatedly below target across the 3 months.")
    ...
    if len(consistently_low) >= 2:
        decision_triggers.append({
            ...
            "description": "Two or more productivity measures were below target in at least 2 of the last 3 months.",
        })
    if isinstance(q_band, str) and q_band.lower() in {"bronze", "below"}:
        decision_triggers.append({
            "title": "Bonus band is low for this period",
            "description": "Overall performance is dragging the bonus band down. Improving productivity will help lift the overall score and bonus.",
        })
    ...
    return {
        ...
        "diagnostics": {
            ...
            "quarter_aggregate": {
                "productivity_pool_score": q_pool,

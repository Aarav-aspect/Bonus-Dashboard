...
def get_job_history_closed_query(start_iso: str, end_iso: str) -> str:
    # NOTE: Salesforce history objects often cannot be filtered by NewValue/OldValue.
    # We intentionally pull all Status__c history rows in the date window and filter in Python.
    return f"""
...
# Keep this function if you want it for reference, but do NOT use it in code
# because many orgs reject filtering History objects by NewValue in query().
def get_job_history_status_changes_query(start_iso: str, end_iso: str, new_values_str: str) -> str:
    """
    new_values_str must be a SOQL tuple string like: ('Quote Sent','Closed')
    WARNING: Many Salesforce orgs disallow filtering History objects by NewValue.
    """
    return f"""
        SELECT Id, ParentId, CreatedDate, Field, OldValue, NewValue
        FROM Job__History
        WHERE Field = 'Status__c'
        AND CreatedDate >= {start_iso} AND CreatedDate < {end_iso}
        AND NewValue IN {new_values_str}
    """
...
def get_jobs_by_ids_query(id_str: str) -> str:
    return f"""
        SELECT Id, Name, Job_Type_Trade__c, Type__c, Status__c, Charge_Policy__c,
               ...
               Sector_Type__c, Account_Type__c, Charge_Net__c
        FROM Job__c
        WHERE Id IN ({id_str})
...
def get_service_appointments_month_query(trades_str: str, start_iso: str, end_iso: str) -> str:
    # IMPORTANT: includes Signed_SR__c for engineer attribution in insights
    return f"""
        SELECT Id, Job__c, Status, CreatedDate, Signed_SR__c,
               Job__r.Job_Type_Trade__c,
               Job__r.Sector_Type__c, Job__r.Account_Type__c
        FROM ServiceAppointment
        WHERE CreatedDate >= {start_iso} AND CreatedDate < {end_iso}
